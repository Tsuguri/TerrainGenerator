...
void ShowError(char* text, UINT type=0)
{
	MessageBoxA(NULL, text, "Turbo Engine", MB_OK | type);
}
...
int ShaderUtility::CompileShader(char* path, GLenum type, bool debugMode)
{
	...
	{
		ShowError("Unable to read and parse shader", MB_ICONERROR);
		return 0;
	}
	else if (debugMode)
	...
	if (!success)
	{
		...
		ShowError("Shader compiled", MB_ICONINFORMATION);
	return shaderId;
